...
void main() {
  test('every setting pack has at least five tables', () {
    for (final style in WorldStyle.values) {
      expect(
        TableLibrary.forStyle(style).length,
    ...
    for (final style in WorldStyle.values) {
      ...
      for (final t in pack) {
        for (final ref in t.references) {
          expect(
            TableLibrary.resolveLocal(style, ref),
            isNotNull,
            reason: '${t.id} -> $ref',
          );
  ...
  test('every pack uses inline dice, alternatives and a weighted table', () {
    for (final style in WorldStyle.values) {
      ...
      expect(
        texts.any((s) => _inlineShape(s).any((x) => !x.startsWith('c'))),
        isTrue,
        reason: style.name,
      );
      expect(
      ...
      expect(
        pack.where((t) => t.formula.isNotEmpty).length,
  ...
  test('formula tables cover the whole die without gaps or overlaps', () {
    for (final t in TableLibrary.all) {
      ...
      expect(
        t.rows.length,
        lessThanOrEqualTo(bounds!.max - bounds.min + 1),
  ...
  test('dependencies follow references transitively', () {
    for (final t in TableLibrary.all) {
      ...
      for (final d in deps) {
        expect(
          ids.containsAll(d.references.difference({t.localId})),
          isTrue,
          reason: '${t.id} via ${d.id}',
        );
      }
    }
    final rumors = TableLibrary.resolveLocal(
      WorldStyle.fantasy,
      'tavern_rumors',
    )!;
    expect(
      [for (final d in TableLibrary.dependencies(rumors)) d.localId],
